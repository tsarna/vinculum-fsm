...
- **Topic matching routes through `vinculum-bus/topicmatch`** — FSM event topic dispatch now honors MQTT 5.0 §4.7.2: filters starting with `+` or `#` no longer match reserved `$`-prefixed topics. Exact and `$`-prefixed patterns are unaffected. Requires vinculum-bus v0.12.0.
